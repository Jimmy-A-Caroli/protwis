...
def create_or_update_residue(protein, segment, schemes,residue,b_and_c):
    ...
    if 'generic_number' in numbers:
        numbers = format_generic_numbers(protein.residue_numbering_scheme, schemes,
    ...
    if 'equivalent' in numbers:
        try:
            gn_equivalent, created = ResidueGenericNumberEquivalent.objects.get_or_create(
                ...
                scheme=protein.residue_numbering_scheme,
    ...
    if 'display_generic_number' in numbers:
        ns = protein.residue_numbering_scheme.slug
        ...
        if gnl in schemes[ns]['generic_numbers']:
            ...
            try:
                ...
                gn = ResidueGenericNumber.objects.get(
                    scheme=protein.residue_numbering_scheme, label=gnl)
    ...
    bulk_r = Residue(protein=protein,sequence_number=sequence_number, amino_acid = rvalues['amino_acid'],
...
def get_gprotein_non_gns(consensus_prot_conf, segment, residues_to_update):
    ...
    non_gns = []
    for p in proteins:
        resis = Residue.objects.filter(protein_segment=segment, protein=p)
        if len(resis)>s_len:
            s_len = len(resis)
    ...
    if len(residues_to_update)!=len(non_gns):
        new_non_gns = []
        resis = Residue.objects.filter(protein_segment=segment, protein__species__common_name='Human',
                                       protein__accession__isnull=False, protein__family__parent=consensus_prot_conf.protein.family)
        for r in resis:
...
def create_or_update_residues_in_segment(protein, segment, start, aligned_start, end, aligned_end,
    ...
    residues_to_update = Residue.objects.filter(Q(sequence_number__gte=start) & Q(sequence_number__lte=end),
        protein=protein).values_list('sequence_number', 'amino_acid')
    ...
    if disregard_db_residues or not residues_to_update.exists():
        residues_to_update = []
        for i, aa in enumerate(protein.sequence[(start-1):end]):
    ...
    if signprot:
        non_gns = get_gprotein_non_gns(protein, segment, residues_to_update)
    created_residues = 0
    for res_num, residue in enumerate(residues_to_update, start=1):
        ...
        if (segment.slug in settings.REFERENCE_POSITIONS
            ...
            numbers = format_generic_numbers_old(protein.residue_numbering_scheme, schemes,
            ...
            if 'equivalent' in numbers:
                try:
                    ...
                    if created:
                        logger.info('Created generic number equivalent {} ({}) for scheme {}'.format(
                            numbers['equivalent'], numbers['generic_number'],
                            protein.residue_numbering_scheme))
                except IntegrityError:
                    try:
                        print("Get",rvalues['generic_number'],protein.residue_numbering_scheme,)
                        gn_equivalent = ResidueGenericNumberEquivalent.objects.get(
                            default_generic_number=rvalues['generic_number'],
                            scheme=protein.residue_numbering_scheme)
                    except:
                        print("failed gn_equivalent", rvalues['generic_number'], protein.residue_numbering_scheme)
        ...
        # UPDATE or CREATE the residue
        r, created = Residue.objects.update_or_create(protein=protein,
            sequence_number=sequence_number, defaults = rvalues)
        if created:
            created_residues += 1
    ...
    if created_residues:
        logger.info('Created {} residues for {} of {}'.format(created_residues, segment, protein))
...
def dgn(gn, protein):
    ...
    return Residue.objects.get(protein=protein, generic_number__label=convert_gn).display_generic_number.label

class DummyResidue():
    def __init__(self, sequence_number, amino_acid, three_letter, display_generic_number=None, generic_number=None, protein=None, protein_segment=None):
