...
class StructureCoverageModelStatisticsQuery:
    ...
    def _struct_count(self, state_slug=None, extra_q=None):
        qs = Structure.objects.filter(
            protein_conformation__protein__parent=OuterRef('pk'),
            # structure_type__slug__in=self.LABORATORY_STRUCTURE_SOURCE,
        )
        if state_slug:
    ...
    def _best_resolution(self, state_slug=None):
        qs = Structure.objects.filter(
            protein_conformation__protein__parent=OuterRef('pk'),
    ...
    def _transducer_count(self, category, family_slug_prefix=None):
        qs = StructureExtraProteins.objects.filter(
            structure__protein_conformation__protein__parent=OuterRef('pk'),
            category=category,
            # structure__structure_type__slug__in=self.LABORATORY_STRUCTURE_SOURCE,
        )
    ...
    def _ligand_type_count(self, type_slugs):
        return (
    ...
    def _role_count(self, role_names, invert=False):
        ...
        return (qry
                #.filter(structure__structure_type__slug__in=self.LABORATORY_STRUCTURE_SOURCE)
                .values('structure__protein_conformation__protein__parent')
                .annotate(c=Count('id'))
                .values('c')
        )   

    def get_primary_gene_name_subquery(self):
        return Subquery(
            Gene.objects.filter(proteins=OuterRef('pk'), position=0).order_by('entrez_id').values('name')[:1]
        )
    
    def get_primary_gene_entrezid_subquery(self):
        return Subquery(
    ...
    #------------#
    @staticmethod
    def statistics_query(self, database):
        ...
        queryset = Protein.objects.filter(
            ...
            gene_name = self.get_primary_gene_name_subquery(),
            gene_entrez_id = self.get_primary_gene_entrezid_subquery(),
            species_common_name = F('species__common_name'),
            
            # ── structure counts by state ──────────────────────────────
            st_inact_c=Coalesce(Subquery(
                self._struct_count('inactive'), output_field=IntegerField()), 0),
            st_interm_c=Coalesce(Subquery(
                self._struct_count('intermediate'), output_field=IntegerField()), 0),
            st_act_c=Coalesce(Subquery(
                self._struct_count('active'), output_field=IntegerField()), 0),

            # ── best (lowest) resolution by state ─────────────────────
            best_res_inact=Subquery(
                self._best_resolution('inactive'), output_field=DecimalField()),
            best_res_interm=Subquery(
                self._best_resolution('intermediate'), output_field=DecimalField()),
            best_res_act=Subquery(
                self._best_resolution('active'), output_field=DecimalField()),
            ...
            transd_gio_c=Coalesce(Subquery(
                self._transducer_count('G alpha', '100_001_002'), output_field=IntegerField()), 0),
            transd_gq11_c=Coalesce(Subquery(
                self._transducer_count('G alpha', '100_001_003'), output_field=IntegerField()), 0),
            transd_g1213_c=Coalesce(Subquery(
                self._transducer_count('G alpha', '100_001_004'), output_field=IntegerField()), 0),
            transd_arrest_c=Coalesce(Subquery(
                self._transducer_count('Arrestin'), output_field=IntegerField()), 0),
            transd_grks_c=Coalesce(Subquery(
                self._grk_count(), output_field=IntegerField()), 0),

            # ── ligand type counts ─────────────────────────────────────
            ligand_types_smmol_c=Coalesce(Subquery(
                self._ligand_type_count(['small-molecule']), output_field=IntegerField()), 0),
            ligand_types_pep_c=Coalesce(Subquery(
                self._ligand_type_count(['peptide', 'protein']), output_field=IntegerField()), 0),

            # ── modality group counts ──────────────────────────────────
            limodgrp_stim_c=Coalesce(Subquery(
                self._role_count(self.STIMULATORY_ROLES), output_field=IntegerField()), 0),
            limodgrp_inhib_c=Coalesce(Subquery(
                self._role_count(self.INHIBITORY_ROLES), output_field=IntegerField()), 0),
            limodgrp_unk_c=Coalesce(Subquery(
                self._role_count(['Unknown']), output_field=IntegerField()), 0),
            limodgrp_other_c=Coalesce(Subquery(
                self._role_count(self.STIMULATORY_ROLES + self.INHIBITORY_ROLES + ['Unknown'], invert=True), output_field=IntegerField()), 0),

            # ── individual modality counts ─────────────────────────────
            limod_apo_c=Coalesce(Subquery(
                self._role_count(['Apo']), output_field=IntegerField()), 0),
            limod_ag_c=Coalesce(Subquery(
                self._role_count(['Agonist']), output_field=IntegerField()), 0),
            limod_ag_partial_c=Coalesce(Subquery(
                self._role_count(['Partial agonist']), output_field=IntegerField()), 0),
            limod_allo_antag_c=Coalesce(Subquery(
                self._role_count(['Allosteric antagonist']), output_field=IntegerField()), 0),
            limod_antag_c=Coalesce(Subquery(
                self._role_count(['Antagonist']), output_field=IntegerField()), 0),
            limod_inv_ag_c=Coalesce(Subquery(
                self._role_count(['Inverse agonist']), output_field=IntegerField()), 0),
            limod_nam_c=Coalesce(Subquery(
                self._role_count(['NAM']), output_field=IntegerField()), 0),
            limod_pam_c=Coalesce(Subquery(
                self._role_count(['PAM']), output_field=IntegerField()), 0),
