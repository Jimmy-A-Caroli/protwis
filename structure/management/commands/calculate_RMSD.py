...
class Validation():
    ...
    def run_RMSD_list(self, files, protein, seq_nums=None, force_chain=None, superpose_on=False, only_backbone=False):
        ...
        if protein.family.slug.startswith('00'):
            base_nums = Residue.objects.filter(protein=protein, protein_segment__slug__in=['TM1', 'TM2', 'TM3', 'TM4', 'TM5', 'TM6', 'TM7']).values_list('sequence_number', flat=True)
            base_type = '7TM'
        # Base definition of G proteins: HN and H5
        elif protein.family.slug.startswith('100'):
            base_nums = Residue.objects.filter(protein=protein, protein_segment__slug__in=['H5']).values_list('sequence_number', flat=True)
            base_type = 'HN,H5'
