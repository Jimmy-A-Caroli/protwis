...
(function ($, window, document) {
  ...
  const columns = [
    // RECEPTOR
    { data: "id", name: "Select", orderable: false, searchable: false,
      render: (data) => `<input type="checkbox" class="select-row" value="${data}">` },
    { data: "gpcrdb_link", name: "GPCRdb", orderable: false, searchable: false,
      render: (d, t) => (t === 'display' && d ? `<a href="${d}" target="_blank" rel="noopener"><img class="gpcrdb-link" src="/static/home/logo/gpcr/main.png" width="12" height="12" alt="GPCRdb"></a>` : '') },
    { data: "Gene", name: "Gene" },
    { data: "entry_short", name: "UniProt",
      render: (d, t, r) => t === 'display' && r ? `<a href="${r.uniprot_link}" target="_blank">${d}</a>` : d },
    { data: "iuphar_name", name: "Protein",
      render: (d, t, r) => {
        if (t !== 'display') return d;
        const url = r && r.iuphar_link;
        return (url && url !== "-") ? `<a href="${url}" target="_blank" rel="noopener">${d}</a>` : d;
      }},
    ...
    { data: "pdb", name: "PDB",
      render: (d,t) => t!=='display' ? d : (d && d!=="-" ? `<a href="https://gpcrdb.org/structure/${d}" target="_blank" rel="noopener">${d}</a>` : d) },
    { data: "refined", name: "Refined structure",
      render: (d, t) => d === "-" ? "-" : (t === 'display' ? `<a href="${d}" target="_blank">${d.replace("refined/", "").toUpperCase()}_refined</a>` : d.replace("refined/", "").toUpperCase() + "_refined") },
    ...
    { data: "arrestin_name", name: "Subtype",
      render: (d,t,r) => t!=='display' ? d : (r?.arrestin_entry && r.arrestin_entry!=="-" ? `<a href="https://gpcrdb.org/signprot/${r.arrestin_entry}/" target="_blank" rel="noopener">${d}</a>` : d) },
    ...
    { data: "ligands", name: "Name",
      render: (d, t) => expandFirstThenHoverLinkList(d, t, id => `https://gpcrdb.org/ligand/${id}/info`, { showCountHint: true }) },
