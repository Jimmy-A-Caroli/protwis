...
def PdbTableData(request):
    ...
    if effector:
        ...
        'extra_proteins','extra_proteins__wt_protein').order_by(
        ...
        signal_ps = StructureExtraProteins.objects.filter(category__startswith=effector).values(
                            'structure__pdb_code__index','structure__protein','structure__protein__parent','display_name',
        ...
        data = data.prefetch_related('extra_proteins','extra_proteins__wt_protein').order_by(
        'extra_proteins__protein__parent','state').annotate(
        res_count = Sum(Case(When(extra_proteins__residue__generic_number=None, then=0), default=1, output_field=IntegerField())))
        signal_ps = StructureExtraProteins.objects.all().exclude(category__in=['G beta','G gamma']).values(
                            'structure__pdb_code__index','structure__protein__parent','display_name',
                            'wt_coverage','wt_protein__family__parent__parent__name','wt_protein__family__parent__name','category','note'
                            ).order_by().annotate(coverage = Max('wt_coverage'))
        ep = {s['structure__pdb_code__index']:s for s in signal_ps}
    ...
    proteins_pks = Structure.objects.all().exclude(structure_type__slug__startswith='af-').values_list("protein__parent__pk", flat=True).distinct()
    proteins_af_pks = Structure.objects.all().filter(structure_type__slug__startswith='af-').values_list("protein__pk", flat=True).distinct()
    if effector:
        proteins_pks = list(proteins_pks) + list(proteins_af_pks)
    residue_counts = Protein.objects.filter(protein__pk__in=proteins_pks).values('protein__pk').annotate(res_count = Sum(Case(When(residue__generic_number=None, then=0), default=1, output_field=IntegerField())))
    rcs = {}
    ...
    if effector:
        resolutions = Structure.objects.all().values('protein__parent','state__name').order_by().annotate(res = Min('resolution'))
    else:
        resolutions = Structure.objects.all().exclude(structure_type__slug__startswith='af-').values('protein__parent','state__name').order_by().annotate(res = Min('resolution'))
    best_resolutions = {}
    ...
    for ps in signal_ps:
        if effector:
            key = '{}_{}_{}'.format(ps['structure__protein'], ps['structure__protein__parent'], ps['display_name'])
        else:
...
#            <th>TM6 tilt (%, inactive: 0-X, intermed: X-Y, active Y-Z)</th> \
    ...
    for s in data:
        ...
        if pdb_id in ep:
            if effector:
                try:
                    key = '{}_{}_{}'.format(s.protein.pk, s.protein.parent.pk, ep[pdb_id]['display_name'])
                except:
                    key = '{}_{}_{}'.format(s.protein.pk, s.protein.parent, ep[pdb_id]['display_name'])
            else:
                key = '{}_{}'.format(s.protein.parent.pk,ep[pdb_id]['display_name'])
    ...
    if data==None:
        ...
        if class_complex_interactions == None or len(class_complex_interactions) == 0:
            ...
            interactions = Interaction.objects.filter(
                ...
                interacting_pair__res__protein_id1=F('interacting_pair__res2__protein_id') # Filter interactions with other proteins
