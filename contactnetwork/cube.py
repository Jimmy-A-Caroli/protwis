...
def compute_interactions(pdb_name, protein=None, signprot=None, lig=None, do_interactions=False, do_complexes=False, do_peptide_ligand=False, save_to_db=False, file_input=False):
    ...
    if file_input:
        ...
        if do_complexes:
            residues = protein.residue_set.all().prefetch_related('generic_number')
        else:
            residues = protein.residue_set.exclude(generic_number=None).all().prefetch_related('generic_number')
        struc = protein
        ...
        if do_complexes:
            residues = struc.residue_set.all().prefetch_related('generic_number')
        else:
            residues = struc.residue_set.exclude(generic_number=None).all().prefetch_related('generic_number')
    ...
    if do_complexes:
        try:
            ...
            residues_sign = Protein.objects.get(entry_name=pdb_name+extension).residue_set.exclude(generic_number=None).prefetch_related('generic_number')
