...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for slug in class_slugs:
            ...
            structure_ids = list(Structure.objects.filter(protein__family__slug__startswith=slug[0]) \
        ...
        if not options["states_only"]:
            ...
            Structure.objects.filter(protein__family__slug__startswith="00").update(representative=False)


            # Select all GPCR structures and get unique slug-state combinations
            struct_combs = list(Structure.objects.filter(protein__family__slug__startswith="00") \
                                .exclude(structure_type__slug__startswith='af-') \
                                .values_list("protein__family__slug", "state", "pk", "resolution", "protein__pk", "protein__parent__pk"))
